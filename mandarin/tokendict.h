...
struct CharAt
{
    ...
    */
    ...
    {
        return memcmp ( &ch, &s[at], sizeof ( TChar ) ) < 0; // /*(ushort)*/ch < /*(ushort)*/cOther;//default signed char is nasty
    }
};
...
{
    return ( uint ) ( s[ca.at] ) < ( uint ) ca.ch;
}

template <class TChar>
inline CharAt<TChar> charAt ( TChar ch, int pos )
{
    return CharAt<TChar> ( ch, pos );
}
...
struct TokenDict
{
    ...
    const vector<const TChar*>& keys() const
    {
        return _keys;
    }
    const vector<TVal>& values() const
    {
        return _vals;
    }
    TVal matchStart ( const TChar*& pStr );
private:
    vector<const TChar*> _keys;
    vector<size_t> _sizes;
    vector<TVal> _vals;
    bool _caseSensitive;//TODO:
    static const char _allZero[sizeof ( TVal )];
    static const TVal& _default;
};

template<class TVal, class TChar>
const char TokenDict<TVal,TChar>::_allZero[] = {};

//I know this is ugly, but I have to make it works for all types
template<class TVal,class TChar>
const TVal& TokenDict<TVal,TChar>::_default = * ( TVal* ) _allZero;
...
template<class TVal, class TChar>
TVal TokenDict<TVal, TChar>::matchStart ( const TChar*& pStr )
{
    ...
    {
        return _vals[candidate - _keys.begin()];
    }
    else if ( ( *head ) [pos] == '\0' ) //empty key matches any thing!
    {
        return _vals[head - _keys.begin()];
    }
    else
    {
        return _default;
